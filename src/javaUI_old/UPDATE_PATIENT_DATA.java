...
//                        jTextField.setText(rsn.getString(9));
                ...
                ResultSet wardAssignedDets = stm.executeQuery("select * from ward_assigned where SSN='" + SSN + "'");
                int fetched_id = 0;
                if (wardAssignedDets.next()) {
                    fetched_id = wardAssignedDets.getInt("triage_id");
                }
                ResultSet rspc = stm.executeQuery("select triage_id, triage from triage");
//                int triage_id = 1;
//                ArrayList<String> triageArray = new ArrayList<String>();
//                triageArray[0] = "";

                while (rspc.next()) {
                    int triage_id = rspc.getInt("triage_id");
                    String patient_con = rspc.getString("triage");
                    jComboBox4.addItem(patient_con);
                    if (fetched_id == triage_id) {
                        jComboBox4.setSelectedItem(patient_con);
                    }
                }
                //To fetch ward_assigned details
...
//                    ResultSet rsnr = stm.executeUpdate("update ward_assigned set ward_id='" + ward_id + "', start_date='" + start_date + "', end_date='" + end_date + "', lastUpdated='" + new Timestamp(System.currentTimeMillis()) + "' where SSN='" + SSN + "'");
                }
